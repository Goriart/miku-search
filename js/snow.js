...
// 在窗口大小改变时重新计算并设置画布的宽度和高度，并更新雪花数组中的雪花位置 此处为gpt3.5添加，用来解决窗口放大后雪花拉伸
	...
	function gameloop() {
		for (var i = 0; i < hearts.length; i++) {
			...
			hearts[i].el.style.cssText = "left:" + hearts[i].x + "px;top:" + hearts[i].y + "px;opacity:" + hearts[i]
				.alpha + ";transform:scale(" + hearts[i].scale + "," + hearts[i].scale + ") rotate(5deg);color:" +
				hearts[i].color;
		}
